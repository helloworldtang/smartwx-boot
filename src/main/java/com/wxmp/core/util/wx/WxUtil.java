...
public class WxUtil {
    ...
    public static JSONObject prepareMenus(List<AccountMenu> menus, Matchrule matchrule) {
        ...
        if (CollectionUtils.isNotEmpty(menus)) {
            List<AccountMenu> parentAM = new ArrayList<>();
            Map<Long, List<JSONObject>> subAm = new HashMap<>();
            for (AccountMenu m : menus) {
                Long parentId = m.getParentId();
                if (parentId != null && parentId.intValue() == 0) {//一级菜单
                    parentAM.add(m);
                } else {//二级菜单
                    if (subAm.get(parentId) == null) {
                        subAm.put(parentId, new ArrayList<>());
                    }
                    List<JSONObject> tmpMenus = subAm.get(parentId);
                    tmpMenus.add(getMenuJSONObj(m));
                    subAm.put(parentId, tmpMenus);
                }
            }
