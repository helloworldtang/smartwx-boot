...
public class WxApiClient {
    ...
    // 获取accessToken
    public static String getAccessToken(String appid, String appsecret, String account)
            throws WxErrorException {
        // 获取唯一的accessToken，如果是多账号，请自行处理
        AccessToken token = WxMemoryCacheClient.getAccessToken(account);
        if (token != null && !token.isExpires() && WxApi.getCallbackIp(token.getAccessToken())) {// 不为空，并且没有过期
            ...
            if (token != null) {
                if (token.getErrcode() == null) {
                    WxMemoryCacheClient.addAccessToken(account, token);
                    return token.getAccessToken();
                } else {
    ...
    public static String getJSTicket(MpAccount mpAccount)
        ...
        if (jsTicket != null && !jsTicket.isExpires()) {// 不为空，并且没有过期
            ...
            String token = getAccessToken(mpAccount.getAppid(), mpAccount.getAppsecret(), mpAccount.getAccount());
    ...
    public static JSONObject createUserTag(String userTags, MpAccount mpAccount) throws WxErrorException {
        String accessToken = getAccessToken(mpAccount.getAppid(), mpAccount.getAppsecret(), mpAccount.getAccount());
